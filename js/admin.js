...
async function checkAuthSession() {
    try {
        ...
        // If session exists, verify that user is an admin via profiles.is_admin
        if (session) {
            const uid = session.user.id;
            const isAdmin = await checkIfAdmin(uid);
            if (!isAdmin) {
                alert(currentLang === 'ar' ? 'هذا الحساب غير مخول بدخول لوحة الأدمن.' : 'This account is not authorized for admin access.');
        ...
        toggleUIState(!!session);
    } catch (err) {
        console.error("Session check failure:", err);
        toggleUIState(false);
    }
    
    // Register session changes listener
    supabase.auth.onAuthStateChange((event, session) => {
        toggleUIState(!!session);
    });
}

// Check profiles.is_admin for the given uid. Returns boolean.
async function checkIfAdmin(uid) {
    try {
        const { data, error } = await supabase.from('profiles').select('is_admin').eq('id', uid).single();
        if (error) {
            console.error('Admin check select error:', error);
            return false;
        }
...
async function loginAdmin(e) {
    ...
    try {
        ...
        if (error) throw error;
        // Auth state listener handles toggle
    } catch (err) {
        console.error("Login failed:", err);
        alert(currentLang === "ar" ? "فشل تسجيل الدخول. يرجى التحقق من صحة البيانات." : "Login failed. Please check your credentials.");
...
function renderOrdersTable() {
    ...
    tbody.innerHTML = currentOrders.map(order => {
        ...
        const totalCost = parseFloat(order.total_price) + parseFloat(order.shipping_cost);
...
function renderBooksTable() {
    ...
    tbody.innerHTML = currentBooks.map(book => {
        ...
        let coverHtml = "";
        if (book.image_url) {
            coverHtml = `<img src="${book.image_url}" alt="${book.title}" class="admin-table-img">`;
        } else {
            coverHtml = `<div class="admin-table-img" style="background:var(--forest); display:flex; align-items:center; justify-content:center; color:var(--gold); font-size:16px; font-weight:700;"><i class="fa-solid fa-book-open"></i></div>`;
        }
...
function closeAddBookModal() {
    ...
    document.getElementById("uploadPromptText").style.display = "block";
}
...
async function submitNewBook(e) {
    ...
    try {
        ...
        // Insert book info in books table
