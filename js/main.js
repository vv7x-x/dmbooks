...
function addToCart(bookId) {
    ...
    updateCartCount();
    renderCart();
    showToast(currentLang === "ar" ? "تم إضافة الكتاب للسلة" : "Book added to cart");
    // toggleCartDrawer(true); // Optional: disabled auto-open to be less intrusive
}
...
function toggleWishlistItem(bookId) {
    ...
    const inWish = wishlist.some(item => item.id === bookId);
    localStorage.setItem("wishlist", JSON.stringify(wishlist));
    updateWishlistCount();
    renderWishlist();
    renderBooksGrid();
    showToast(inWish ? (currentLang === "ar" ? "تم الإضافة للمفضلة" : "Added to wishlist") : (currentLang === "ar" ? "تم الحذف من المفضلة" : "Removed from wishlist"));
}
